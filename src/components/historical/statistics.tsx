...
const StatBox = ({ label, value, compare }: { label: string; value: string; compare?: string }) => (
  ...
  >
    ...
    {compare && (
      <div
        className="text-[10px] font-bold uppercase tracking-wider mt-2 pt-2"
        style={{
  ...
  const stats = useMemo(() => {
    ...
    const median =
      sortedCloses.length % 2 !== 0
        ? sortedCloses[mid].close
        : (sortedCloses[mid - 1].close + sortedCloses[mid].close) / 2

    const variance = data.reduce((acc, d) => acc + Math.pow(d.close - average, 2), 0) / data.length
    const stdDev = Math.sqrt(variance)

    const dailyReturn = calcReturns(data, 1)
    const monthlyReturn = calcReturns(data, 21)
    const yearlyReturn = calcReturns(data, 252)
